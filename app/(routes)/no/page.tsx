...
const ExploreCard: React.FC<ExploreCardProps> = ({ href, title, description, isEnglish }) => (
  <Link 
    ...
    className="bg-slate-50 dark:bg-slate-800/90 p-5 rounded-lg 
      shadow-sm hover:shadow-md transition-all duration-300 
      hover:-translate-y-1 border border-transparent 
      dark:border-slate-700/50 hover:border-slate-200 
      dark:hover:border-slate-600"
  >
    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2 transition-colors duration-200">
      {title}
    </h3>
    <p className="text-slate-600 dark:text-slate-300 text-sm transition-colors duration-200">
      {description}
    </p>
  </Link>
);
        ...
        <div className="flex flex-col md:flex-row items-center">
          <div className="p-6 md:p-8 md:w-3/5">
            ...
            <div className="space-y-3 md:space-y-0 md:space-x-3 md:flex">
              <Link 
                ...
                className="block md:inline-block text-center px-4 py-2 
                  bg-slate-600 dark:bg-slate-700 
                  text-slate-50 dark:text-slate-50 
                  rounded hover:bg-slate-700 dark:hover:bg-slate-600 
              ...
              <Link 
                ...
                className="block md:inline-block text-center px-4 py-2 
                  ...
                  rounded hover:bg-blue-200 dark:hover:bg-slate-700 
                  transition-colors duration-200 text-sm font-medium">
                {t.home.cta.about}
              </Link>
            </div>
          ...
          <div className="p-6 md:w-2/5 flex justify-center">
            <div className="relative w-72 h-72 overflow-hidden rounded-full 
              border-4 border-blue-50/90 dark:border-slate-600 
              ...
              <Image
                ...
                quality={100}
                loading="eager"
                className="rounded-full object-cover object-center absolute inset-0 w-full h-full scale-105"
              />
            </div>
          </div>
        </div>
